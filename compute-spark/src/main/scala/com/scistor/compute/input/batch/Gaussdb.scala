...
class Gaussdb extends Jdbc {
  ...
  override def initProp(driver: String): (Properties, Array[String]) = {
    println(s"$this , ${extraProp.getProperty("timeunit")}")
      ...
      case _ => {
        ...
        if (timeunit != null) {
          ...
          predicates = tuples.map(elem => {
            s"cast($partColumnName as timestamp) >= timestamp '${elem._1}' and cast($partColumnName as timestamp) < timestamp '${elem._2}'"
          }).toArray
        } else {
          // 取模的方式划分分区
